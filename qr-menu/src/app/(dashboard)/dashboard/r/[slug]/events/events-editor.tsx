    ...
    <div className="space-y-4">
      ...
      {items.length === 0 ? (
        <Card className="p-12 text-center text-sm text-muted-foreground">
          Расскажите про 3–4 ближайших события — кино, бранч, концерт, лекцию.
        </Card>
      ) : (
            ...
            <Card key={it.id} className="p-4 flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1 text-xs text-muted-foreground">
                  <CalendarDays className="h-3 w-3" /> {it.whenText}
                </div>
                <div className="font-medium truncate mt-0.5">{it.title}</div>
                {it.description && (
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editing?.mode === "edit" ? "Редактировать событие" : "Новое событие"}
            </DialogTitle>
            <DialogDescription>
              Кнопка «Записаться» откроет WhatsApp с темой события.
            </DialogDescription>
          </DialogHeader>
          <form
          ...
          >
            <div className="space-y-1.5">
              ...
              <Input id="title" name="title" required defaultValue={editing?.mode === "edit" ? editing.item.title : ""} placeholder="Кинопоказ «Амели»" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="whenText">Когда</Label>
              <Input id="whenText" name="whenText" required defaultValue={editing?.mode === "edit" ? editing.item.whenText : ""} placeholder="Сб, 10 мая, 20:00" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="description">Описание (по желанию)</Label>
              <Textarea id="description" name="description" rows={2} defaultValue={editing?.mode === "edit" ? editing.item.description ?? "" : ""} placeholder="Бесплатно, по записи. Аренда пуфика — 500₽." />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input id="whatsappTopic" name="whatsappTopic" defaultValue={editing?.mode === "edit" ? editing.item.whatsappTopic ?? "" : ""} placeholder="Хочу записаться на «Амели» 10 мая" />
              <p className="text-xs text-muted-foreground">
                Если не указать — будет «Здравствуйте, хочу записаться на: [название события]».
              </p>
            </div>
      ...
      <AlertDialog open={!!confirmDelete} onOpenChange={(o) => !o && setConfirmDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Удалить событие «{confirmDelete?.title}»?</AlertDialogTitle>
            <AlertDialogDescription>
              Оно пропадёт с витрины.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
